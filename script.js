...
function addRecentBasic(item, b) {
	...
	el.innerHTML = '<div class="recent' + (b ? ' bb' : '') + '"><b class="theB"' + (localStorage.getItem('astiw_markread') != 'true' && localStorage.getItem('astiw_viewed' + id) == 'true' ? ' style="opacity:0.5;"' : '') + '>' + trimTitle(title.replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/&/g, '&amp;')) +'</b></div>';
	main.appendChild(el);
}
...
function generateAttachmentElement(data, usage) {
	...
	if (type == 'image') {
		...
		if (usage == 'editor') {
			...
			el.innerHTML = '<h3 style="margin:0;"><a class="classic" target="_blank" style="margin-top:0;" href="' + data + '">Open file</a></h3><p class="small" style="margin-bottom:0;">If you cannot view the file by clicking the link, try right clicking the link and choosing "Open in new tab" (or equivalent)</p>';
